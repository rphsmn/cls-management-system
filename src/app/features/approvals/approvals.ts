...
export class ApprovalsComponent {
  ...
  constructor() {
    this.allFilteredRequests$ = combineLatest([
      ...
      map(
        ([user, allRequests, searchTerm, selMonth, selYear]: [
          User | null,
          any[],
          string | null,
          number | null,
          ...
          return allRequests.filter((req) => {
            ...
            if (
              normalizedTarget === 'admin manager' &&
              adminManagerRoles.includes(normalizedUserRole)
            ) {
              return true;
            }
            ...
            const opsAdminRoles = [
              ...
              'ops admin supervisor',
              'ops admin',
            ];
            if (
              normalizedTarget === 'operations admin supervisor' &&
              opsAdminRoles.includes(normalizedUserRole)
            ) {
              return true;
            }
            ...
            if (
              normalizedTarget === 'account supervisor' &&
              acctSupervisorRoles.includes(normalizedUserRole)
            ) {
              return true;
            }
  ...
  private checkPeriodMatch(period: string, selMonth: number, selYear: number): boolean {
    ...
    return (
      (startDate.getMonth() === selMonth && startDate.getFullYear() === selYear) ||
      (endDate.getMonth() === selMonth && endDate.getFullYear() === selYear)
    );
  }

  getFormattedPeriod(req: any): string {
    if (!req?.period) return 'N/A';

    // Check for half-day leave
    if (req.isHalfDay || req.daysDeducted === 0.5) {
      const separator = req.period.includes(' to ') ? ' to ' : ' - ';
      const dateStr = req.period.split(separator)[0].trim();
      const singleDate = new Date(dateStr);
      return `Half Day (${singleDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })})`;
    }
  ...
  viewDocument(attachment: any) {
    ...
    newTab?.document.write(
      `<iframe src="${attachment.data}" frameborder="0" style="width:100%; height:100%;"></iframe>`,
    );
  }
  ...
  toggleReason(req: any) {
    this.expandedReq = this.expandedReq === req ? null : req;
  }
