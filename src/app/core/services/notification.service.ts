...
import { Firestore, collection, addDoc, query, where, orderBy, onSnapshot, Unsubscribe, getDocs, writeBatch, doc, getDoc, updateDoc, deleteDoc } from '@angular/fire/firestore';
import { BehaviorSubject, Observable } from 'rxjs';
import { environment } from '../../../environments/environment';

export interface Notification {
  id?: string;
  type: 'new_leave_request' | 'leave_approved' | 'leave_rejected';
  title: string;
  message: string;
  fromUser: string;
  fromUserId: string;
...
export class NotificationService {
  ...
  subscribeToNotifications(userRole: string, userUid: string) {
    ...
    // Simplified query - get all notifications for target roles and filter in memory
    ...
    this.unsubscribe = onSnapshot(q, (snapshot) => {
      // Get ALL notifications and sort by date (newest first)
      const notifications = snapshot.docs
        .map(doc => {
          const data = doc.data() as any;
          const createdAt = data.createdAt?.toDate ? data.createdAt.toDate() : new Date(data.createdAt?.seconds ? data.createdAt.seconds * 1000 : Date.now());
          return {
            id: doc.id,
            type: data.type,
            title: data.title,
  ...
  private getTargetRolesForRole(role: string): string[] {
    ...
    if (['PART-TIME', 'EMPLOYEE', 'IT DEVELOPER', 'IT ASSISTANT', 'SENIOR IT DEVELOPER',
         'ADMIN OPERATION OFFICER', 'ADMIN OPERATION ASSISTANT', 'ADMIN COMPLIANCE OFFICER',
         'ACCOUNTING CLERK', 'ACCOUNT RECEIVABLE SPECIALIST', 'ACCOUNT PAYABLES SPECIALIST'].includes(upperRole)) {
      return [];
    }
  ...
  async notifyNewLeaveRequest(
    ...
    console.log('[NotificationService] Creating notification for leave request:', { employeeName, employeeRole, leaveType });
    
    const targetRoles = this.getNotificationTargets(employeeRole);
    console.log('[NotificationService] Notification targets for role', employeeRole, ':', targetRoles);
  ...
  private getNotificationTargets(employeeRole: string): string[] {
    ...
    if (['ADMIN OPERATION OFFICER', 'ADMIN OPERATION ASSISTANT', 'ADMIN COMPLIANCE OFFICER'].includes(upperRole)) {
      return ['OPERATIONS ADMIN SUPERVISOR', 'HR', 'HUMAN RESOURCE OFFICER'];
    }
    
    // Accounts staff -> Account Supervisor, then HR
    if (['ACCOUNTING CLERK', 'ACCOUNT RECEIVABLE SPECIALIST', 'ACCOUNT PAYABLES SPECIALIST'].includes(upperRole)) {
